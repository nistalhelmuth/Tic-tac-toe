...
function finalizado(jugador) { 
  //definir el boton para repetir juego
  ...
  // eliminar propiedad de onclick en el tablero (no pude solo eliminar el eventListener)
  ...
  lState.positionsy.forEach(function(posy) {
    ...
    lState.positionsx.forEach(function(posx) {
      ...
      positionElement.addEventListener('click',function myOnclick() {
        if(!positionElement.classList.contains('circle') 
        && !positionElement.classList.contains('cross')
      ) {
          ...
          for (let i = 0; i<3; i++) {
            ...
            for (let o = 1; o<3; o++) {
              if (jugador != lState.tableboard[i][o] 
                || jugador == undefined
              ) {
                ganar=false;
              }
          ...
          for (let i = 0; i<3; i++) {
            ...
            for (let o = 1; o<3; o++) {
              if (jugador != lState.tableboard[o][i] 
                || jugador == undefined
              ) {
                ganar=false;
              }
          ...
          for (let i = 1; i<3; i++) {
            if (
              jugador != lState.tableboard[i][2-i] 
              || jugador == undefined
            ) {
                ganar=false;
              }
